...
public class ReminderService {
	...
	private ReminderResponse createResponse(ReminderEntity reminder) {
		return ReminderResponse.builder()
				...
				.repeatUnit(reminder.getRepeatUnit())
	...
	public List<ReminderResponse> getAllReminders(int page,int size){
		
		 	Pageable pageable = PageRequest.of(page, size);
		 	UserEntity user = authService.getCurrentUser();		 	

		    Page<ReminderEntity> reminders = reminderRepo.findByStatusAndUser(pageable,ReminderStatus.PENDING,user);

		    return reminders.getContent().stream()
		            .map(reminder -> createResponse(reminder))
		            .toList();
	}
	...
	public void updateReminder(Long reminderId, ReminderUpdateRequest dto) {
	    ...
	    ReminderEntity reminder = reminderRepo
	            .findByReminderIdAndDeletedAtIsNull(reminderId)
	            .orElseThrow(() ->
